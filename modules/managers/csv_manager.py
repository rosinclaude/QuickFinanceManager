...
class CSVManager:
    ...
    def save_payee_data(self, payee_name: str, is_subscription: bool = False, notes: str = ""):
        """
        Internal helper to save a payee to the payees.csv if it doesn't already exist.
        Excludes payees that are identified as internal accounts.
        """
        ...
        is_internal_account_payee = any(
            keyword in payee_name for keyword in internal_account_keywords
        ) or (payee_name in self.load_transactions()[
            'Account'].unique().tolist())  # Check if it's an existing account name

        if is_internal_account_payee:
            print(f"'{payee_name}' identified as internal account payee. Not saving to payees.csv.")
            return

        payees_df = self.load_payees()
        if payee_name and payee_name not in payees_df['Name'].values:
            print(f"Adding new payee: {payee_name}")
            new_payee = {
                "Name": payee_name,
                "Aliases": "",
                "DefaultCategory": "",
            ...
            new_df = pd.DataFrame([new_payee])
            updated_df = pd.concat([payees_df, new_df], ignore_index=True)
            self.save_payees(updated_df)
        else:
            print(f"Payee '{payee_name}' already exists or is empty. Not adding.")